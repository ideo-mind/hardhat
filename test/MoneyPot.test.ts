...
describe("MoneyPot", function () {
  ...
  async function deployMoneyPotFixture() {
    ...
    const [deployer, user1, user2, user3, feeRecipient] =
  ...
  describe("Deployment", function () {
    ...
    it("Should set the correct minimum deposit", async function () {
      const { moneyPot, minimumDeposit } = await loadFixture(
    ...
    it("Should set the correct maximum deposit", async function () {
      const { moneyPot, maximumDeposit } = await loadFixture(
        deployMoneyPotFixture
      )
    ...
    it("Should set the correct fee percentage", async function () {
      const { moneyPot, feePercentage } = await loadFixture(
        deployMoneyPotFixture
      )
    ...
    it("Should set the correct fee recipient", async function () {
      const { moneyPot, feeRecipient } = await loadFixture(
        deployMoneyPotFixture
      )

      const recipient = await moneyPot.read.feeRecipient()
      expect(getAddress(recipient)).to.equal(
  ...
  describe("Deposits", function () {
    it("Should accept valid deposits", async function () {
      const { moneyPot, user1, publicClient } = await loadFixture(
        deployMoneyPotFixture
      )
    ...
    it("Should collect fees correctly", async function () {
      const { moneyPot, user1, feeRecipient, publicClient } = await loadFixture(
        deployMoneyPotFixture
      )
    ...
    it("Should emit Deposited event", async function () {
      ...
      const logs = await moneyPot.getEvents.Deposited(
        {},
  ...
  describe("Withdrawals", function () {
    it("Should allow users to withdraw their balance", async function () {
      ...
      const balanceBefore = await moneyPot.read.balances([
    ...
    it("Should emit Withdrawn event", async function () {
      ...
      const receipt = await publicClient.waitForTransactionReceipt({ hash })
      const logs = await moneyPot.getEvents.Withdrawn(
        {},
        { fromBlock: receipt.blockNumber }
      )
      ...
      expect(logs[0].args.amount).to.be.closeTo(
        withdrawAmount,
        parseEther("0.001")
      )
    })
  ...
  describe("Emergency Withdraw", function () {
    it("Should allow emergency withdrawal even when paused", async function () {
      const { moneyPot, user1, deployer } = await loadFixture(
        deployMoneyPotFixture
      )
    ...
    it("Should emit EmergencyWithdraw event", async function () {
      ...
      const receipt = await publicClient.waitForTransactionReceipt({ hash })
      const logs = await moneyPot.getEvents.EmergencyWithdraw(
        {},
        { fromBlock: receipt.blockNumber }
      )
  ...
  describe("Admin Functions", function () {
    ...
    it("Should allow owner to update fee recipient", async function () {
      const { moneyPot, deployer, user3 } = await loadFixture(
        deployMoneyPotFixture
      )
    ...
    it("Should reject fee percentage above 10%", async function () {
      ...
      await expect(
        moneyPot.write.setFeePercentage([1001n], {
          // 10.01%
          account: deployer.account,
        })
  ...
  describe("Pausable", function () {
    ...
    it("Should reject deposits when paused", async function () {
      const { moneyPot, deployer, user1 } = await loadFixture(
        deployMoneyPotFixture
      )
  ...
  describe("View Functions", function () {
    it("Should return correct balance for user", async function () {
      ...
      const directBalance = await moneyPot.read.balances([
        user1.account.address,
      ])
    ...
    it("Should return correct pot statistics", async function () {
      const { moneyPot, user1, user2, publicClient } = await loadFixture(
        deployMoneyPotFixture
      )
